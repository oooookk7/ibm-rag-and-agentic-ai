...
def _extract_json_from_text(text: str):
    ...
    # Try grabbing the first JSON object.
    start = cleaned.find("{")
    end = cleaned.rfind("}")
...
def format_recipe_output(final_output):
    ...
    if "recipes" in final_output:
        ...
        elif isinstance(recipe_task_output, dict):
            if "recipes" in recipe_task_output:
                recipes = recipe_task_output.get("recipes", [])
            elif isinstance(recipe_task_output.get("raw"), str):
                parsed = _extract_json_from_text(recipe_task_output["raw"])
    ...
    if recipes:
        ...
        raw = final_output.get("raw") or final_output.get("output")
...
def format_analysis_output(final_output):
    ...
    if not final_output.get("dish"):
        parsed = _extract_json_from_text(final_output.get("raw", ""))
        if isinstance(parsed, dict):
            final_output = parsed
    ...
    if health_eval := final_output.get('health_evaluation'):
        output += "\n**Health Evaluation:**\n\n"
        output += health_eval + "\n"
    elif isinstance(final_output.get("raw"), str):
        output += "\n" + final_output["raw"]
